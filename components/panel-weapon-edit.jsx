...
import {findByProperty, indexOfByProperty, pluck, sortByProperty} from '../lib/list';
...
export default class PanelWeaponEdit extends React.Component {
  ...
  render() {
    ...
    let form = <div>
      ...
      <Select label="Skill" value={this.state.skill} values={this.skills} handler={this.setSkill.bind(this)}
              required={true}/>
      <Select label="Range" value={this.state.range} values={this.ranges} handler={this.setRange.bind(this)}
              required={true}/>
      <TextInput label="Damage" value={this.state.damage} handler={this.setDamage.bind(this)} required={true}
                 note="Remember to add Brawn to damage for Melee or Brawl weapons."/>
      <TextInput label="Critical" value={this.state.critical} handler={this.setValue('critical').bind(this)}/>
      <SelectQuality label="Qualities" value={this.state.qualities} values={qualities}
                     select={this.toggleQuality.bind(this)} update={this.updateQuality.bind(this)}/>
      <TextArea label="Notes" value={this.state.notes} handler={this.setValue('notes').bind(this)}/>
      <PanelCode/>
      <button className="btn-full" disabled={!this.state.canSave} onClick={this.create.bind(this)}>{button} Weapon
      </button>
    </div>;
    ...
    return <div>
      {this.props.editing
        ...
        <div>
          <h3>Select Weapon</h3>
          <AutoComplete label="Weapon" value={selected} values={list} handler={this.selectItem.bind(this)}
                        required={true}/>
          <button className="btn-full" disabled={!this.state.selected} onClick={this.add.bind(this)}>Add Selected
            Weapon
          </button>
