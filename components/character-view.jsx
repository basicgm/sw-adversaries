...
import {getSourceLink, hashToArray, symbolise} from '../lib/utils';
...
export default class CharacterView extends React.Component {
  ...
  render() {
    ...
    if (character.tags.indexOf('rebel alliance') != -1) {
      icon = <svg>
        <use xlinkHref="#rebel-alliance"></use>
      </svg>;
    } else if (character.tags.indexOf('empire') != -1) {
      icon = <svg>
        <use xlinkHref="#galactic-empire"></use>
      </svg>;
    } else if (character.tags.indexOf('first order') != -1) {
      icon = <svg>
        <use xlinkHref="#first-order"></use>
      </svg>;
    }
    ...
    let favLink = character.favourite
      ? <li onClick={this.removeFavourite.bind(this)}>
        <svg className="star-filled outline">
          <use xlinkHref="#icon-star-full"></use>
        </svg>
        Remove Favourite</li>
      : <li onClick={this.addFavourite.bind(this)}>
        <svg>
    ...
    let copyLink = character.id.startsWith(CONFIG.ADVERSARY_ID)
      ? [<li onClick={this.copyAdversary.bind(this)}>
        <svg>
          <use xlinkHref="#icon-edit"></use>
        </svg>
        Edit</li>, <li onClick={this.deleteAdversary.bind(this)}>
        <svg>
    ...
    if (character.favourite) {
      favIcon = <svg className="star-filled outline">
        <use xlinkHref="#icon-star-full"></use>
      </svg>;
    }
    ...
    let derived = vehicle
      ? <div className="stats vehicle" id="derived">
        ...
        <div>
          ...
          <form onSubmit={this.setCurrentWounds.bind(this)}><input type="text" placeholder={this.state.currentWounds}
                                                                   maxLength="2" ref="currentWounds"/></form>
        </div>
        <div><h3>System Strain <small>Threshold | Current</small></h3>
          <span>{character.type === CONFIG.MINION ? vehicle.derived.system * this.state.minions : vehicle.derived.system} |</span><input
            type="text" defaultValue="0" maxLength="2"/></div>
        <div className="small">
      ...
      : <div className="stats" id="derived">
        ...
        {character.type === CONFIG.NEMESIS ?
          <div><h3>Strain <small>Threshold | Current</small></h3><span>{character.derived.strain} |</span><input
            type="text" defaultValue="0" maxLength="2"/></div> : null}
        <div>
    ...
    return <div className={!this.props.visible ? 'hidden' : null}>
      ...
      <h2 className="subtitle">
        ...
        <span className={'btn ' + activeMenuState} onClick={this.toggleCharacterMenu.bind(this)}><svg><use
          xlinkHref="#icon-menu"></use></svg></span>
        <div id="menu-character" className={activeMenuState}>
          <ul>
            <li onClick={this.createAdversary.bind(this)}>
              <svg>
      ...
      {character.notes ?
        <div className="text" dangerouslySetInnerHTML={symbolise(this.md.render(`*${character.notes}*`))}></div> : null}
      ...
      <div className="column large">
        <PanelSkill character={character} skills={this.props.skills} aliveMinions={this.state.aliveMinions}
                    minions={this.state.minions} setMinions={this.setMinions.bind(this)} boost={boost}
                    setback={setback}/>
        <PanelWeapons title="Weapons" character={character} vehicle={vehicle} skills={this.props.skills}
                      weapons={this.props.weapons} qualities={this.props.qualities} talents={this.props.talents}
                      aliveMinions={this.state.aliveMinions} minions={this.state.minions}/>
        {vehicle
          ? <PanelVehicle vehicle={vehicle} onClose={this.removeVehicle.bind(this)}/>
          : <div className="info">
            <h2>Vehicle</h2>
            <SelectGroup value={vehicle ? vehicle.name : null}
                         values={this.props.vehicles.all().sort(sortByProperty('name'))} display="name" groupBy="group"
                         handler={this.selectVehicle.bind(this)}/>
            <div className="row-input">
              <button className="btn-full" disabled={this.state.selectedVehicle == ''}
                      onClick={this.addVehicle.bind(this)}>Add Vehicle
              </button>
            </div>
          </div>}
